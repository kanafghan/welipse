...
import com.github.kanafghan.welipse.webdsl.diagram.edit.commands.InternalLinkTargetCreateCommand;
import com.github.kanafghan.welipse.webdsl.diagram.edit.commands.InternalLinkTargetReorientCommand;
 ...
 * @generated
 */
public class InternalLink2ItemSemanticEditPolicy extends
		WebdslBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public InternalLink2ItemSemanticEditPolicy() {
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (WebdslVisualIDRegistry.getVisualID(incomingLink) == InternalLinkTargetEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (WebdslVisualIDRegistry.getVisualID(outgoingLink) == InternalLinkTargetEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (WebdslVisualIDRegistry.getVisualID(node)) {
			case InternalLinkInternalLinkSourceCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (WebdslVisualIDRegistry.getVisualID(cnode)) {
					case Text4EditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it
								.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (WebdslVisualIDRegistry
									.getVisualID(incomingLink) == InternalLinkTargetEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (WebdslElementTypes.InternalLinkTarget_4004 == req.getElementType()) {
			return getGEFWrapper(new InternalLinkTargetCreateCommand(req,
	...
	protected Command getReorientReferenceRelationshipCommand(
		...
		case InternalLinkTargetEditPart.VISUAL_ID:
			return getGEFWrapper(new InternalLinkTargetReorientCommand(req));
		}
